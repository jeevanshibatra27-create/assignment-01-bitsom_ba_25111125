...
Example from the dataset:
Columns such as customer_id, customer_name, customer_email, and customer_city appear in many rows. If a customer changes their city, every row containing that customer must be updated. If one row is missed, inconsistent data will appear in the table.
...
Example from the dataset:
Columns product_id, product_name, category, and unit_price are stored in the same row as order_id. If the only order containing a specific product is deleted, the product information will also be lost from the table.
